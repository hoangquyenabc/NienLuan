<?php
    ...
    $id = $_GET['id'];
    // Truy vấn lấy thông tin đơn hàng, địa chỉ giao hàng và khách hàng
    $sql = "SELECT d.*, h.*, k.*
    ...
    $shipping_fee = 35000;
    // Truy vấn cơ sở dữ liệu để lấy danh sách sản phẩm trong đơn hàng
    $sql_cart = "
    ...
    if (isset($_POST['update_status'])) {
        // Kiểm tra trạng thái được gửi từ form
        if (isset($_POST['order_status'])) {
            $new_status = mysqli_real_escape_string($conn, $_POST['order_status']);
            $id_order = intval($id); // ID đơn hàng
        ...
        // Nếu đơn hàng bị hủy, cập nhật lý do hủy
        if ($new_status === "Đã hủy") {
            $update_sql = "UPDATE don_hang SET TrangThai_DH = '$new_status', LyDoHuy = '$cancel_reason' WHERE ID_DH = $id_order";
        } else {
        ...
        if ($result_update) {
            ...
            echo '<script>window.location.href = window.location.href;</script>'; // Làm mới trang
    
            // Nếu đơn hàng bị hủy, gửi email thông báo
            if ($new_status === "Đã hủy") {
