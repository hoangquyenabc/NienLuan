...
class UserController{
    ...
    public function getFriendList($query): string
    {
        ...
        while ($row = mysqli_fetch_assoc($query)) {
            // Nếu là khách hàng, chỉ lấy bạn bè là nhân viên
            if ($role == "0") {  // Khách hàng
                // Lọc ra các nhân viên (role = 1)
                if ($row['vaitro'] != "1") { // Nếu không phải nhân viên
                    continue;  // Bỏ qua, không hiển thị
                }
            } elseif ($role == "1") {  // Nếu là nhân viên
                // Nhân viên có thể thấy tất cả khách hàng
                if ($row['vaitro'] != "0") { // Nếu không phải khách hàng
                    continue;  // Bỏ qua, không hiển thị
                }
            }
