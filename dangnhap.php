               ...
               if (isset($_POST['dangnhap'])) {
                if (empty($_POST['email']) || empty($_POST['password'])) {
                    ...
                    if (mysqli_num_rows($query_admin) > 0) {
                        ...
                        $_SESSION['role'] = $row_admin['VaiTro']; // Lấy vai trò từ cơ sở dữ liệu
                    
                        // Kiểm tra vai trò và chuyển hướng
                        if ($_SESSION['role'] === 'Admin') {
                            header('Location: admin/customer.php'); // Chuyển đến trang quản lý admin
                        } elseif ($_SESSION['role'] === 'Staff') {
                            header('Location: admina/customer.php'); // Chuyển đến trang nhân viên
                        } else {
                            echo "Vai trò không hợp lệ!";
                            exit();
                        }
                        
                        exit(); // Ngăn việc tiếp tục thực hiện code sau redirect
                    } else {
