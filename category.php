...
if (isset($_POST['atcicon'])) {
    ...
    foreach ($cart as &$item) {
        if ($item['ID_SP'] == $id) {
            $item['quantity'] += $quantity;  // Cập nhật số lượng nếu sản phẩm đã có trong giỏ hàng
            $isFound = true;
            break;
        }
    }
    ...
    if (!$isFound) {
        // Thực hiện truy vấn để lấy thông tin sản phẩm cụ thể theo ID_SP
        $sql_str = "SELECT sp.ID_SP, sp.Ten_SP, bg.Gia, bg.NgayGio, sp.HinhAnh
                    ...
                    WHERE sp.ID_L = {$category_id} AND sp.ID_SP = {$id}";  // Lọc sản phẩm theo ID danh mục và ID sản phẩm
    ...
    // Điều hướng lại trang để cập nhật giỏ hàng
    echo '<script>window.location.href = window.location.href;</script>';
}
?>

<?php 
// Truy vấn lấy danh sách sản phẩm theo ID danh mục
$sql = "SELECT sp.ID_SP, sp.Ten_SP, bg.Gia, bg.NgayGio, sp.HinhAnh, MAX(dh.ID_DH) AS ID_DH, l.*, dh.ID_DH
                    FROM san_pham sp
                    JOIN (
                        SELECT ID_SP, Gia, NgayGio
                        FROM co_gia bg1
